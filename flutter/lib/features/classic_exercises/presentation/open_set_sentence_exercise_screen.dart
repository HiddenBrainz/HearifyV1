...
/// Sentence Comprehension — Level 3 (Open Set). Plays each sentence via
/// TTS, the user speaks it back. Scoring via [pronunciationSimilarity]
/// — sentences are harder to speak verbatim than single words so the
/// pass threshold is a touch lower than Word Recognition's.
class OpenSetSentenceExerciseScreen extends ConsumerStatefulWidget {
  const OpenSetSentenceExerciseScreen({super.key});

  @override
  ConsumerState<OpenSetSentenceExerciseScreen> createState() =>
      _OpenSetSentenceExerciseScreenState();
}

class _OpenSetSentenceExerciseScreenState
    extends ConsumerState<OpenSetSentenceExerciseScreen> {
  ...
  Widget build(BuildContext context) {
    final b = Theme.of(context).brightness;
    final accent = SentenceComprehensionLevel.openSet.color(b);
            ...
            : _current == null
                ? Center(
                    child: Padding(
                      ...
                      child: Text(
                        'No sentences found. Check that '
                        'SentenceComprehensionData.csv is bundled.',
          ...
          ModernCard(
            child: Column(
              children: [
                ...
                Text(
                  stt.isRecording
                      ? 'Listening — say the full sentence'
                      : _revealed
                          ? 'Tap play to hear it again'
                          : 'Tap play, then speak the sentence back',
                  textAlign: TextAlign.center,
                  style: TextStyle(
                    fontSize: 13,
                    color: AppTheme.textSecondary(b),
